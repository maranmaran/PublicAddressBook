...
{
    public class GetNotificationHistoryRequestHandler : IRequestHandler<GetNotificationHistoryRequest, IEnumerable<Notification>>
    {
        private readonly IApplicationDbContext _context;
        private readonly IS3Service _s3Service;

        public GetNotificationHistoryRequestHandler(IApplicationDbContext context, IS3Service s3Service)
        {
            _context = context;
            _s3Service = s3Service;
        }

        public async Task<IEnumerable<Notification>> Handle(GetNotificationHistoryRequest request, CancellationToken cancellationToken)
        {
            try
            {
                var notifications = await _context.Notifications
                        .Include(x => x.Sender)
                        .Where(x => x.ReceiverId == request.UserId || x.ReceiverId == null)
